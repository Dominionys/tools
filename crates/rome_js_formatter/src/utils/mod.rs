...
struct LiteralStringCleaner<'token> {
...
impl<'token> LiteralStringCleaner<'token> {
    pub fn new(
    ...
    fn compute_preferred_quote(&mut self) {
        ...
        let alternate = self.chosen_quote_style.other();
        ...
        let (preferred_quotes_count, alternate_quotes_count) = literal.chars().enumerate().fold(
            ...
            |(preferred_quotes_counter, alternate_quotes_counter), (index, current_character)| {
                if index == 0 {
                    let chosen_quote_char = self.chosen_quote_style.as_char();
                    ...
                    return (preferred_quotes_counter, alternate_quotes_counter);
                } else if index == char_count - 1 {
                    return (preferred_quotes_counter, alternate_quotes_counter);
                }
                if current_character == self.chosen_quote_style.as_char() {
        ...
        if preferred_quotes_count > alternate_quotes_count {
            ...
            self.preferred_quote = Some(self.chosen_quote_style);
        }
...
impl Format for FormatLiteralStringToken<'_> {
    ...
    fn format(&self, formatter: &Formatter<Self::Options>) -> FormatResult<FormatElement> {
        ...
        let mut string_cleaner = LiteralStringCleaner::new(self, chosen_quote_style);
